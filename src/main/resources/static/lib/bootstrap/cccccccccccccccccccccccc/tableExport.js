    ...
    $.fn.tableExport = function(options) {
        var defaults = {
            ...
            jspdf: {
                ...
                unit: "pt",
                format: "a4",
                // One of jsPDF page formats or 'bestfit' for autmatic paper format selection
                margins: {
                    left: 20,
                    right: 10,
                    top: 10,
                ...
                autotable: {
                    styles: {
                        ...
                        fontStyle: "normal",
                        // normal, bold, italic, bolditalic or 'inherit' to use css font-weight and fonst-style from html table
                        overflow: "ellipsize",
                    ...
                    tableExport: {
                        doc: null,
                        // jsPDF doc object. If set, an already created doc will be used to export to
            ...
            numbers: {
                ...
                output: {
                    // Use 'output: false' to keep number format in exported output
                    decimalMark: ".",
                    thousandsSeparator: ","
                }
            },
            ...
            onCellHtmlData: null,
            onIgnoreRow: null,
            // onIgnoreRow($tr, rowIndex): function should return true to not export a row
            outputMode: "file",
            // 'file', 'string', 'base64' or 'window' (experimental)
            pdfmake: {
                enabled: false,
                // true: use pdfmake instead of jspdf and jspdf-autotable (experimental)
                docDefinition: {
                    pageOrientation: "portrait",
        ...
        $.extend(true, defaults, options); // Adopt deprecated options
        ...
        if (
            typeof defaults.excelFileFormat !== "undefined" &&
            defaults.mso.fileFormat === "undefined"
        )
            defaults.mso.fileFormat = defaults.excelFileFormat;
        if (
            typeof defaults.excelPageFormat !== "undefined" &&
            defaults.mso.pageFormat === "undefined"
        )
            defaults.mso.pageFormat = defaults.excelPageFormat;
        if (
            typeof defaults.excelPageOrientation !== "undefined" &&
            defaults.mso.pageOrientation === "undefined"
        )
            defaults.mso.pageOrientation = defaults.excelPageOrientation;
        if (
            typeof defaults.excelRTL !== "undefined" &&
            defaults.mso.rtl === "undefined"
        )
            defaults.mso.rtl = defaults.excelRTL;
        if (
            typeof defaults.excelstyles !== "undefined" &&
            defaults.mso.styles === "undefined"
        )
            defaults.mso.styles = defaults.excelstyles;
        if (
            typeof defaults.onMsoNumberFormat !== "undefined" &&
            defaults.mso.onMsoNumberFormat === "undefined"
        )
            defaults.mso.onMsoNumberFormat = defaults.onMsoNumberFormat;
        if (
            typeof defaults.worksheetName !== "undefined" &&
            defaults.mso.worksheetName === "undefined"
        )
            defaults.mso.worksheetName = defaults.worksheetName; // Check values of some options

        defaults.mso.pageOrientation =
            defaults.mso.pageOrientation.substr(0, 1) === "l"
                ? "landscape"
                : "portrait";
        ...
        if (
            ...
            var csvString = function csvString(cell, rowIndex, colIndex) {
                ...
                if (cell !== null) {
                    ...
                    if (defaults.type === "tsv") {
                        if (_instanceof(dataString, Date)) dataString.toLocaleString(); // According to http://www.iana.org/assignments/media-types/text/tab-separated-values
                        ...
                        if (_instanceof(dataString, Date))
                            result =
            ...
            findTableElements($(el), "tbody").each(function() {
                rowlength += CollectCsvData(
                    findTableElements($(this), defaults.tbodySelector),
                    "td,th",
                    rowlength
                );
            });
            ...
            $(el)
                ...
                .each(function() {
                    ...
                    var ssName = "";
                    if (
                        typeof defaults.mso.worksheetName === "string" &&
                        defaults.mso.worksheetName.length
                    )
                        ssName = defaults.mso.worksheetName + " " + (docNames.length + 1);
                    else if (
                        typeof defaults.mso.worksheetName[docNames.length] !== "undefined"
                    )
                            ...
                            ForEachVisibleCell(
                                ...
                                function(cell, row, col) {
                                    if (cell !== null) {
                                        ...
                                        var rowspan = getRowspan(cell); // Skip spans
                    ...
                    var rowLength = CollectXmlssData(
            ...
            defaults.type === "excel" &&
            defaults.mso.fileFormat === "xlsx"
        ) {
            var docNames = [];
            var workbook = XLSX.utils.book_new(); // Multiple worksheets and .xlsx file extension #202
            ...
            $(el)
                ...
                .each(function() {
                    ...
                    if (
                        ...
                        sheetName =
                            defaults.mso.worksheetName + " " + (docNames.length + 1);
                    else if (
                        typeof defaults.mso.worksheetName[docNames.length] !== "undefined"
                    )
                    ...
                    if (!sheetName.length) sheetName = "Table " + (docNames.length + 1);
                    sheetName = $.trim(
                        sheetName.replace(/[\\\/[\]*:?'"]/g, "").substring(0, 31)
                    );
            ...
            $(el)
                ...
                .each(function() {
                    ...
                    rowIndex = 0;
                    ranges = [];
                    colNames = GetColumnNames(this); // Header
                            ...
                            function(cell, row, col) {
                                if (cell !== null) {
                                    ...
                                    if (
                                        ...
                                        typeof defaults.mso.onMsoNumberFormat === "function"
                                    )
            ...
            docFile +=
                "@page Section1 {size:" +
                pageFormats[defaults.mso.pageFormat][0] +
                "pt " +
                pageFormats[defaults.mso.pageFormat][1] +
                "pt";
            docFile +=
            ...
            docFile +=
                '<div class="Section' +
                (defaults.mso.pageOrientation === "landscape" ? "2" : "1") +
                '">';
            docFile += docData;
            docFile += "</div>";
            docFile += "</body>";
            ...
            if (defaults.pdfmake.enabled === true) {
                ...
                var CollectPdfmakeData = function CollectPdfmakeData(
                    $rows,
                    colselector,
                    length
                ) {
                ...
                var teOptions = defaults.jspdf.autotable.tableExport; // When setting jspdf.format to 'bestfit' tableExport tries to choose
                ...
                loadImages(teOptions, function() {
                    $(el)
                        ...
                        .each(function() {
                            ...
                            teOptions.columns = [];
                            teOptions.rows = [];
                            teOptions.teCells = {}; // onTable: optional callback function for every matching table that can be used
                            // to modify the tableExport options or to skip the output of a particular table
                            // if the table selector targets multiple tables

                            if (typeof teOptions.onTable === "function")
                                if (teOptions.onTable($(this), defaults) === false) return true; // continue to next iteration step (table)
                                    ...
                                    if (
                                        ...
                                        if (typeof col.rect !== "undefined") {
                                            ...
                                            if (
                                                ...
                                                else rh = data.row.raw[data.column.dataKey].rect.height;
                                        ...
                                        cell.styles.halign =
                                            atOptions.headerStyles.halign === "inherit"
                                                ? "center"
                                                : atOptions.headerStyles.halign;
                            ...
                            if (typeof atOptions.createdCell !== "function") {
                                ...
                                atOptions.createdCell = function(cell, data) {
                                    ...
                                    cell.styles.halign =
                                        atOptions.styles.halign === "inherit"
                                            ? "center"
                                            : atOptions.styles.halign;
                                    cell.styles.valign = atOptions.styles.valign;

                                    if (
                                        typeof tecell !== "undefined" &&
                                        typeof tecell.style !== "undefined" &&
                                        tecell.style.hidden !== true
                                    ) {
                            ...
                            if (typeof atOptions.drawCell !== "function") {
                                atOptions.drawCell = function(cell, data) {
                                    ...
                                    var draw2canvas =
                                        typeof tecell !== "undefined" && tecell.isCanvas;
                                    ...
                                    if (draw2canvas !== true) {
                                        if (prepareAutoTableText(cell, data, tecell)) {
                                            ...
                                            if (
                                                typeof tecell !== "undefined" &&
                                                typeof tecell.elements !== "undefined" &&
                                                tecell.elements.length
                                            ) {
                                ...
                                ForEachVisibleCell(
                                    ...
                                    function(cell, row, col) {
                                        ...
                                        if (typeof cell !== "undefined" && cell !== null) {
                                            ...
                                            obj.isCanvas = cell.hasAttribute(
                                                "data-tableexport-canvas"
                                            );
                                            obj.elements = obj.isCanvas
                                                ? $(cell)
                                                : $(cell).children();
                                            ...
                                            obj = $.extend(
                                                true,
                                                {},
                                                teOptions.teCells[rowCount + ":" + (colKey - 1)]
                                            );
                                            obj.colspan = -1;
                                            teOptions.teCells[rowCount + ":" + colKey++] = obj;
                                        }
        ...
        function collectHeadRows($table) {
            ...
            findTableElements($table, "thead").each(function() {
                result.push.apply(
                    result,
                    findTableElements($(this), defaults.theadSelector).toArray()
                );
            });
            return result;
        }
        ...
        function collectRows($table) {
            ...
            findTableElements($table, "tbody").each(function() {
                result.push.apply(
                    result,
                    findTableElements($(this), defaults.tbodySelector).toArray()
                );
            });
            ...
            if (defaults.tfootSelector.length) {
                findTableElements($table, "tfoot").each(function() {
                    result.push.apply(
                        result,
                        findTableElements($(this), defaults.tfootSelector).toArray()
                    );
                });
            }
            ...
            if (typeof cellcallback === "function") {
                ...
                if (
                    ...
                    $cells.each(function(colIndex) {
                        ...
                        var rowspan = getRowspan(this); // Skip ranges
        ...
        function prepareAutoTableText(cell, data, cellopt) {
            ...
            if (cs >= 0) {
                ...
                if (typeof cellopt !== "undefined" && cellopt.rowspan > 1)
                    cell.height = cell.height * cellopt.rowspan; // fix jsPDF's calculation of text position
                        ...
                        $.each(html, function() {
                            if ($(this).is("input"))
                                ...
                                htmlData += $cell
                                    ...
                                    .eq(selectidx++)
                                    .text();
                            else if ($(this).is("br")) htmlData += "<br>";
                            else {
        ...
        function jx_Workbook() {
            if (!_instanceof(this, jx_Workbook)) {
        ...
        function jx_createSheet(data) {
            ...
            for (var R = 0; R !== data.length; ++R) {
                for (var C = 0; C !== data[R].length; ++C) {
                    ...
                    else if (_instanceof(cell.v, Date)) {
                        cell.t = "n";
